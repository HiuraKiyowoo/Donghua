              ...
              liveResults.map((r) => (
                <div 
                  ...
                  onClick={() => { 
                    ...
                    navigate(`/anime/${encodeURIComponent(r.url || r.id)}`); 
                    setIsSearchOpen(false); 
                  }} 
                  className="flex items-center gap-4 p-3 hover:bg-white/5 cursor-pointer border-b border-white/5 transition-colors"
                >
                  <img src={r.cover || r.image} referrerPolicy="no-referrer" className="w-10 aspect-[3/4.5] object-cover rounded-md shadow-md" />
                  <div className="flex flex-col">
                    {/* ✅ FIX: Pakai r.judul */}
                    <span className="text-white font-bold text-xs line-clamp-1">{r.judul || r.title}</span>
